...
def parse_typenames(typename_list):
    ...
    typeid = numpy.array([mapping[t]
                          for t in typename_list], dtype=numpy.uint32)
...
def parse_constraints(obj, constraint_node, n):
    ...
    if len(constraint_list) > 0:
        ...
        constraint_arr = constraint_arr.reshape(
            (len(constraint_list) / (n + 1), n + 1))
...
def read_xml(name):
    ...
    if configuration.hasAttribute('dimensions'):
        snap.configuration.dimensions = int(
            configuration.getAttribute('dimensions'))
    else:
    ...
    snap.configuration.box = numpy.array(
        [lx, ly, lz, xy, xz, yz], dtype=numpy.float32)
    ...
    snap.particles.position = snap.particles.position.reshape(
        (snap.particles.N, 3))
    ...
    if len(type_nodes) == 1:
        ...
        snap.particles.typeid, snap.particles.types = parse_typenames(
            type_names)
    else:
    ...
    if len(moment_inertia_nodes) == 1:
        ...
        snap.particles.moment_inertia = numpy.array(
            moment_inertia_list, dtype=numpy.float32)
        snap.particles.moment_inertia = snap.particles.moment_inertia.reshape(
            (snap.particles.N, 3))
    ...
    if len(image_nodes) == 1:
        ...
        snap.particles.image = snap.particles.image.reshape(
            (snap.particles.N, 3))
    ...
    if len(velocity_nodes) == 1:
        ...
        snap.particles.velocity = numpy.array(
            velocity_list, dtype=numpy.float32)
        snap.particles.velocity = snap.particles.velocity.reshape(
            (snap.particles.N, 3))
    ...
    if len(orientation_nodes) == 1:
        ...
        snap.particles.orientation = numpy.array(
            orientation_list, dtype=numpy.float32)
        snap.particles.orientation = snap.particles.orientation.reshape(
            (snap.particles.N, 4))
    ...
    if len(angmom_nodes) == 1:
        ...
        snap.particles.angmomg = snap.particles.angmomg.reshape(
            (snap.particles.N, 4))
